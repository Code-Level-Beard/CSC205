...
public class Sorting {
	...
	void sort(T[] data) {
		do {
			for (int ii = 0; ii < data.length-1; ii++) {
				if (less_than(data[ii+ 1], data[ii])) {
					swap(data, ii + 1, ii);
				}
			}
	...
	void cutoffQsort(T[] data) {
		int count = 0;
		cutoffQsort(data, 0, data.length-1, count);
	}

	private static <T extends Comparable<T>> 
	void cutoffQsort(T[] data, int min, int max, int count) {
		if (min < max) {
			// Create a partition
			int indexOfPartition = partition(data, min, max);
			count+=1;		
			...
			// sort the left partition (lower values)
			cutoffQsort(data, min, indexOfPartition - 1, count);
			
			// sort the right partition (higher values)
			cutoffQsort(data, indexOfPartition + 1, max, count);
		}
	}
	...
	List<Integer> closestPair(Integer[] data) {
		...
		int x = data[0];
		int y = data[1];
		int minDistance = Math.abs(data[0] - data[1]);
		for (int ii = 0; ii < data.length-1; ii++) {
			if (Math.abs(data[ii] - data[ii+1]) < minDistance) {
				minDistance = Math.abs(data[ii] - data[ii + 1]);
