...
import projects.project8.jsjf.ArrayUnorderedList;
import projects.project8.jsjf.BinaryTreeNode;
...
public class Sorting {
	...
	void bstSort(T[] data) {
		// Project 8
		LinkedBinarySearchTree<T> tempTree = new LinkedBinarySearchTree<T>();
		ArrayUnorderedList<T> tempList = new ArrayUnorderedList<T>();
		
		for (int ii = 0; ii <data.length; ii++) {
			tempTree.addElement(data[ii]);
		}
		
		bstSort(data, tempTree.getRootNode(), tempList);
	}
	
	public static <T extends Comparable<T>> 
	void bstSort(T[] data,BinaryTreeNode<T> node, ArrayUnorderedList<T> tempList) {
		if (node != null) {
		...
		for (int ii = 0; ii < data.length; ii++) {
			data[ii] = tempList.removeFirst();
		}
	}
	
	public static <T> void inOrder(BinaryTreeNode<T> node, 
